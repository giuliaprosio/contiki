...
static struct ctimer restart_ping_pong_timer;

static message_t message;

static int waiting;

static void restart_ping(void *d);
/*---------------------------------------------------------------------------*/
            ...
            uint16_t chunk_len)
{
  ...
  LOG_INFO("Pub handler: topic='%s' (len=%u), chunk_len=%u\n", topic, topic_len,
    ...
    message = *(message_t *) chunk;
    LOG_INFO("[PING] Recive pong message: %d sequance number: %u\n", message.secret, message.seq_nr_value);
    if(message.seq_nr_value < PING_PONG_NUMBER){
        publish();
    }else{
      LOG_INFO("[PING] STOP");
    }
...
{
    int len = snprintf(pub_topic, BUFFER_SIZE, MQTT_DEMO_PUBLISH_TOPIC_PING);
...
{
  int len = snprintf(sub_topic, BUFFER_SIZE, MQTT_DEMO_SUB_TOPIC_PONG);
...
publish(void)
{
  message.seq_nr_value++;
  LOG_INFO("[PING] publishing ping: %d\n", message.seq_nr_value);

 

  mqtt_publish(&conn, NULL, pub_topic, (uint8_t *)&message,
               sizeof(message_t), MQTT_QOS_LEVEL_0, MQTT_RETAIN_OFF);

  LOG_INFO("[PING] Publish sent out!\n");
  LOG_INFO("[PING] on topic %s\n", pub_topic);
  ctimer_set(&restart_ping_pong_timer, conf.pub_interval, restart_ping, NULL);
  waiting = 1;
}
/*---------------------------------------------------------------------------*/
static void
...
static void restart_ping(void *d){
    LOG_INFO("[PING] call restart\n");
    if(waiting && message.seq_nr_value < PING_PONG_NUMBER){
        LOG_INFO("[PING] message lost. Restarting... \n");
        reset_ping();
        publish();
    }else{
        LOG_INFO("[PING] stop \n");
