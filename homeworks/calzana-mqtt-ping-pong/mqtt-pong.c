            ...
            uint16_t chunk_len)
{
    ...
    LOG_INFO("[PONG] Recive ping message: %d sequance number: %u\n", message.secret, message.seq_nr_value);
...
{
    int len = snprintf(pub_topic, BUFFER_SIZE, MQTT_DEMO_PUBLISH_TOPIC_PONG);
...
{
    int len = snprintf(sub_topic, BUFFER_SIZE, MQTT_DEMO_SUB_TOPIC_PING);
...
publish(void)
{
    ...
    LOG_INFO("[PONG] publishing ping: %d\n", message.seq_nr_value);
    ...
    LOG_INFO("[PONG] Publish sent out!\n");
    LOG_INFO("[PONG] on topic %s\n", pub_topic);
}
/*---------------------------------------------------------------------------*/
static void
