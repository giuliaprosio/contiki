...
static int waiting_for_package = 0;

static void ctimer_callback(void *data);
static struct ctimer timer;

#define CTIMER_INTERVAL 3 * CLOCK_SECOND
/*---------------------------------------------------------------------------*/
                ...
                uint16_t datalen)
{
    ...
    if (count == 5) {
        ...
        waiting_for_package = count == 5 ? 0 : 1;
        ctimer_set(&timer, CTIMER_INTERVAL, ctimer_callback, &sender_addr);
    }
}
/*---------------------------------------------------------------------------*/
PROCESS_THREAD(udp_client_process, ev, data)
{
    ...
    while(1) {
        ...
        if(NETSTACK_ROUTING.node_is_reachable() && NETSTACK_ROUTING.get_root_ipaddr(&dest_ipaddr)) {
            ...
            ctimer_set(&timer, CTIMER_INTERVAL, ctimer_callback, &dest_ipaddr);
        } else {
...
static void ctimer_callback(void *data) {
    if (waiting_for_package) {
        printf("client restarting\n");
        unsigned count = 0;
        uip_ipaddr_t * dest_ipaddr = (uip_ipaddr_t *) data;
        simple_udp_sendto(&udp_conn, &count, sizeof(count), dest_ipaddr);
        waiting_for_package = 1;
        ctimer_set(&timer, CTIMER_INTERVAL, ctimer_callback, dest_ipaddr);
    }
